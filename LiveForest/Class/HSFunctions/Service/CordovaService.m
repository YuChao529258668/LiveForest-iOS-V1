...
#import "CordovaService.h"

@implementation CordovaService

#pragma mark 获取CordovaService实例
+ (CordovaService*)getSingletonInstance{
    
    static CordovaService *instance = nil;
    @synchronized(self) {
        if (instance == nil)
...
- (void)startPage:(NSString*)pageName fromViewController:(UIViewController*)fromViewController{
    ...
    //Optional 添加一个返回按钮
    UIButton *btn = [[UIButton alloc]initWithFrame:CGRectMake(0, 20, 30, 40)];//构造一个返回按钮
